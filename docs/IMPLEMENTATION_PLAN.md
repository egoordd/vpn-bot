...
32. **[Codex/Claude]** Выделить общий биллинг-API (бот + сайт). *(Реализованы `services/billing_api.py`, FastAPI transport `services/http_api.py` и Next proxy для account/plans/promo.)*
...
Обновление 2026-06-13 — **пункты 35–37 доведены до UX бота**: кнопка «💰 Кошелёк» (баланс + последние операции), пополнение через CryptoBot (`unlock_topup`-payload, ручной курс `RUB_PER_USDT`, авто-зачисление в поллере с уведомлением), оплата standard-тарифов с баланса в один тап (кнопка в инвойсе при достаточном балансе; авто-возврат `refund` при сбое панели), FSM-ввод промокодов, реферальная кнопка со ссылкой `?start=ref_…` и парсинг реф-кода в /start. CryptoBot-поллер не планируется при пустом токене. Тестовый набор герметизирован от живого `.env` (conftest принудительно очищает панельные/платёжные креды).
- **32 (общий биллинг-API):** `services/billing_api.py` расширен композитным `AccountOverview` (подписка + кошелёк + рефералка) и тонкими обёртками — единая точка входа для бота и сайта. `services/http_api.py` отдаёт `/plans`, `/regions`, `/account/{user_id}`, `/promo/preview`; Next proxy `/api/plans` уже использует live billing API при `BILLING_API_URL` и fallback-каталог в demo-режиме.
...
Осталось по Фазе 3 (требует участия/секретов): реальная auth-сессия сайта вместо demo `userId=1`, checkout-ручки сайта для CryptoBot/баланса/промокода, применение скидки-промокода в самом buy-флоу, **карточный PSP (34)**. Документация контракта — `docs/BILLING_API.md`.
