...
async def test_stats_handler_summarizes(session_pool, monkeypatch):
    ...
    async with session_pool() as session:
        await Repository(session).create_user(telegram_id=1, username="a")
...
@pytest.mark.integration
async def test_gift_promo_mints_a_subscription_code(session_pool, monkeypatch):
    from bot.handlers import admin
    from services.promo import PROMO_SUBSCRIPTION_GRANT

    monkeypatch.setattr(admin.settings, "ADMIN_IDS", "777")
    message = SimpleNamespace(
        text="/gift_promo SUMMER 30 10",
    ...
    assert promo.kind == PROMO_SUBSCRIPTION_GRANT
    assert promo.value == 30
    assert promo.max_uses == 10


@pytest.mark.integration
async def test_gift_promo_refuses_a_length_no_tariff_sells(session_pool, monkeypatch):
    """Rounding to a nearby plan would mint a code that hands out a different
    length than the admin typed."""
    from bot.handlers import admin

    monkeypatch.setattr(admin.settings, "ADMIN_IDS", "777")
    message = SimpleNamespace(
        text="/gift_promo ODDONE 45",
        from_user=SimpleNamespace(id=777, username="adm"),
        answer=AsyncMock(),
    )
    await admin.gift_promo_handler(message, session_pool)

    assert "совпадать с тарифом" in message.answer.await_args.args[0]
    async with session_pool() as session:
        assert await Repository(session).get_promo_code("ODDONE") is None
